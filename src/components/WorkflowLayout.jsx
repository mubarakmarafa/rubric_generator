...
import { generateRubric, normalizeQuestionType } from '../services/openaiService';
...
export default function WorkflowLayout() {
  ...
  const [studentWorkspaceActive, setStudentWorkspaceActive] = useState(false);
  ...
  const handleImageUpload = async (file) => {
    ...
    if (file) {
      try {
        ...
        // Do NOT call generateRubric here - that should only happen in handleWorkflowExecute
      } catch (error) {
  ...
  const handleDetectQuestion = async () => {
    if (!image) {
    ...
    try {
      ...
      for (const line of lines) {
        ...
        if (lowerLine.includes('question:')) {
          questionText = line.substring(line.indexOf(':') + 1).trim();
        } else if (lowerLine.includes('type:')) {
          questionType = line.substring(line.indexOf(':') + 1).trim();
        } else if (lowerLine.includes('format:')) {
          format = line.substring(line.indexOf(':') + 1).trim();
        }
      }

      if (!questionText) {
        throw new Error('Could not detect question text from the image');
      }
      
      // Normalize the question type using the shared function
      const normalizedType = normalizeQuestionType(questionType) || 'short_answer';
      ...
      console.log('Setting detected question:', questionData);
      setDetectedQuestion(questionData);
      
    } catch (error) {
      console.error('Error detecting question:', error);
      alert(error.message || 'Error detecting question. Please try again.');
    } finally {
      setIsDetectingQuestion(false);
    }
  };
  ...
  const handlePromptSubmit = async (prompt) => {
    ...
    if (!image && !detectedQuestion?.text) {
      ...
      alert('Please either upload an image or enter question details');
      return;
    }
  ...
  const handleWorkflowExecute = async () => {
    if (!image && !detectedQuestion) {
      alert('Please either upload an image or enter question details');
      return;
    }

    if (!detectedQuestion) {
      alert('Please detect the question first');
      return;
    }
    ...
    try {
      // Execute the workflow with the detected question
      if (selectedWorkflow) {
        // Sync prompts before running workflow
        const updatedSteps = selectedWorkflow.steps.map((step, idx) => ({
          ...step,
        ...
        <div className="workflow-layout">
          <div className="sidebar">
            <div className="sidebar-top">
              ...
              <div className={`${!isKeySet ? 'disabled' : ''}`}>
                {!detectedQuestion ? (
                  <div className="detected-question-box">
                    <h3>Detected Question</h3>
                    <div className="question-details">
                      <p>No question detected</p>
                      <button 
                        onClick={handleDetectQuestion}
                        className="detect-question-btn"
                        disabled={isDetectingQuestion || !image}
                      >
                        {isDetectingQuestion ? 'Detecting...' : 'Detect Question'}
                      </button>
                    </div>
                  </div>
                ) : (
                  <DetectedQuestion 
                    ...
                    type={detectedQuestion.type?.replace(/_/g, ' ')}
                    format={detectedQuestion.format}
                  />
                )}
              </div>
          ...
          {/* Output Panel with Student Workspace Button */}
          <div className="output-panel">
            ...
            <button
              className="student-workspace-btn"
              style={{ marginTop: '1.5rem', width: '100%', background: rubric ? '#1976d2' : '#ccc', color: 'white', border: 'none', borderRadius: '8px', padding: '1rem', fontWeight: 600, fontSize: '1.1rem', cursor: rubric ? 'pointer' : 'not-allowed', transition: 'background 0.2s' }}
              disabled={!rubric}
              onClick={() => rubric && setStudentWorkspaceActive(true)}
            >
              Student Workspace
            </button>
          </div>
